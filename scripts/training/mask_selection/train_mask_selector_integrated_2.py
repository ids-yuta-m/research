...
def train_mask_selection_model():
    ...
    # plt.subplot(1, 4, 1)
    # plt.plot(train_psnrs, label='Current PSNR', linewidth=2)
    # plt.title('Absolute PSNR Progress')
    # plt.xlabel('Epoch')
    # plt.ylabel('PSNR (dB)')
    # plt.legend()
    # plt.grid(True)
    ...
    plt.subplot(1, 4, 2)
    plt.plot(train_rewards, label='Reward vs Baseline', linewidth=2)
    plt.axhline(y=0, color='r', linestyle='--', alpha=0.7, label='Zero Reward')
    plt.title('Training Reward (vs Moving Baseline)')
    plt.xlabel('Epoch')
